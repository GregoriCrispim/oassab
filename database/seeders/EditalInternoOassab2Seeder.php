...
class EditalInternoOassab2Seeder extends Seeder
{
    public function run(): void
    {
        $dataFile = base_path('app/Data/edital-interno-oassab-2-2026.php');

        if (! file_exists($dataFile)) {
            $this->command?->warn("[EditalInternoOassab2Seeder] Arquivo {$dataFile} não encontrado. Pulei.");
        ...
        if (! is_dir($assetsDir)) {
            $this->command?->warn("[EditalInternoOassab2Seeder] Pasta de assets {$assetsDir} não encontrada. Pulei.");
        ...
        if (! file_exists($mainSource)) {
            $this->command?->warn("[EditalInternoOassab2Seeder] PDF principal não encontrado: {$mainSource}");
        ...
        foreach ($data['attachments'] as $attachmentData) {
            ...
            if (! file_exists($source)) {
                $this->command?->warn("[EditalInternoOassab2Seeder] Anexo não encontrado: {$source}");
                continue;
            }
        ...
        $this->command?->info('[EditalInternoOassab2Seeder] Edital "'.$data['title'].'" publicado em /editais/'.$data['slug']);
    }
