...
5. **Após cada deploy** (obrigatório — o FTP não roda seeders no MySQL de produção):
   ```bash
   php artisan static-content:sync
   ```
   Esse comando grava editais/comunicados estáticos no banco, espelha `storage/app/public` em `public/storage/` e limpa o cache. Se capas de posts antigos sumirem, rode também:
   ```bash
